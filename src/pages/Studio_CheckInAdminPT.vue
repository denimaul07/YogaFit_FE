<script setup>
    ...
    const tampilData = async (page=state.listData.current_page) => {
        ...
        if (user.roles[0].name=='studio') {
            await Api.get('admin/bookingPT?page=' + page+ '&q=' + search.value +  '&studio=' + studio.value+ '&tgl=' + tgl.value)
            .then(response => {
        ...
            }
            ...
            await Api.get('/sales/bookingPT?page=' + page+ '&q=' + search.value +  '&studio=' + studio.value+ '&tgl=' + tgl.value)
            .then(response => {
                state.listData = response.data.data
                loading.value = false
            }).catch(error => {
    ...
    const save = async () => {
        ...
        Api.defaults.headers.common['Authorization'] = "Bearer " +token
        if (user.roles[0].name=='superAdmin' || user.roles[0].name=='admin') {
            await Api.post('admin/bookingPT', {
            ...
            await Api.post('sales/bookingPT', {
                idcontract :  state.data.idcontract,
                start_time : state.data.start_time,
                end_time : state.data.end_time,
                studio : state.data.id_studio,
