    ...
    <div class="grid grid-cols-12 gap-6 mt-5">
        <div class="col-span-12 2xl:col-span-4">
            <div class="flex flex-wrap items-center col-span-12  intro-y sm:flex-nowrap">
                ...
                <RouterLink :to="{ name: 'kat_pembayaran' }" class="flex items-center pr-4 text-primary mt-5"><Lucide icon="ChevronRight" class="w-4 h-4 mr-3" /> Master Kategori Pembayaran</RouterLink>
            </div>
        
        </div>
        <div class="col-span-12 2xl:col-span-4">
            <h2 class="mt-4 text-lg font-medium intro-y items-center text-center">List Data Kategori Pembayaran</h2>
        </div>
        <div class="col-span-12 2xl:col-span-4">
                    ...
                    <div class="items-center sm:flex sm:mr-4">
                        <Button variant="primary" class="mr-2 shadow-md" @click="add">
                            <span class="flex items-center justify-center w-40 h-5">
                                <Lucide icon="Plus" class="w-4 h-4" /> Add New Kategori Pembayaran
                            </span>
                        </Button>
                                        ...
                                        <Table.Td class="first:rounded-l-md last:rounded-r-md w-40 bg-white border-b-0 dark:bg-darkmode-600 shadow-[20px_3px_20px_#0000000b] whitespace-nowrap">
                                            <div
                                                ...
                                                'flex items-center justify-center',
                                                    { 'text-success': data.status_pembayaran==0 },
                                                    { 'text-danger': data.status_pembayaran==1 }
                                                ]"
                                            >
                                                <!-- <Lucide icon="CheckSquare" class="w-4 h-4 mr-2" /> -->
                                                <div v-if="data.status_pembayaran==0">Active</div>
                                                <div v-else-if="data.status_pembayaran==1">Non Active</div>
                    ...
                    <Dialog.Description  class="grid grid-cols-12 gap-4 gap-y-3">
                        <div class="col-span-12 sm:col-span-6 xl:col-span-6 intro-y">
                            <FormInline class="mt-2">
                                <FormLabel  htmlFor="horizontal-form-2" class="sm:w-40" style="text-align: left;"> Kategori Pembayaran </FormLabel>
                                <FormInput type="text" class="col-span-8" v-model="state.data.kat_pembayaran" placeholder="Insert Kategori Pembayaran"/>
                            </FormInline>
                        ...
                        <div class="col-span-12 sm:col-span-6 xl:col-span-6 intro-y">
                            ...
                            <FormInline class="mt-2">
                                ...
                                <FormSelect v-model="state.data.status_pembayaran" class="w-full">
...
<script setup>
    ...
    const setSuccessModalPreview = (value) => {
        ...
        if (successModalPreview.value ==false) {
            ...
            state.data.id=''
            state.data.kat_pembayaran=''
            state.data.status_pembayaran=''
        }
    };
    ...
    const tampilData = async (page = state.listData.current_page) => {
        ...
        const token = localStorage.getItem('token_yogafit')
        Api.defaults.headers.common['Authorization'] = "Bearer " +token
        await Api.get('/admin/katpembayaran?page=' + page+ '&q=' + search.value)
        .then(response => {
    ...
    const edit = async (id) => {
        ...
        Api.defaults.headers.common['Authorization'] = "Bearer " +token
        await Api.get('/admin/katpembayaran', {
            params: {
                id : id
            } 
        })
        .then(response => {
            ...
            state.data.kat_pembayaran=state.datas[0].kat_pembayaran
            state.data.sessions=state.datas[0].sessions
            state.data.status_pembayaran=state.datas[0].status_pembayaran
            action.value ='Update'
            AddModal.value = true
            loadingsycn.value = false
            basicModalPreview.value = false
    ...
    const save = async () => {
        ...
        if (action.value=='Add New') {
            await Api.post('/admin/katpembayaran', {
                kat_pembayaran : state.data.kat_pembayaran,
                status_pembayaran : state.data.status_pembayaran,
            }).then(async (response) => {
                loadingsycn.value = false
                basicModalPreview.value = false
                setSuccessModalPreview(true)
                pesan.value = "Success Add Kategori Pembayaran"
            ...
            await Api.put('/admin/katpembayaran', {
                id : state.data.id,
                kat_pembayaran : state.data.kat_pembayaran,
                status_pembayaran : state.data.status_pembayaran,
            }).then(async (response) => {
            
                pesan.value = "Success Update Kategori Pembayaran"
                setSuccessModalPreview(true)
                loadingsycn.value = false
                basicModalPreview.value = false
                AddModal.value = false
