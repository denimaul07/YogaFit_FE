    ...
    <div class="grid grid-cols-12 gap-6 mt-5">
        <div class="col-span-12 2xl:col-span-4">
            <div class="flex flex-wrap items-center col-span-12  intro-y sm:flex-nowrap">
                ...
                <RouterLink :to="{ name: 'product' }" class="flex items-center pr-4 text-primary mt-5"><Lucide icon="ChevronRight" class="w-4 h-4 mr-3" /> Master Product</RouterLink>
            </div>
        
        </div>
        <div class="col-span-12 2xl:col-span-4">
            <h2 class="mt-4 text-lg font-medium intro-y items-center text-center">List Data Product</h2>
        </div>
        <div class="col-span-12 2xl:col-span-4">
                    ...
                    <div class="items-center sm:flex sm:mr-4">
                        <Button variant="primary" class="mr-2 shadow-md" @click="add">
                            <span class="flex items-center justify-center w-40 h-5">
                                <Lucide icon="Plus" class="w-4 h-4" /> Add New Product
                            </span>
                        </Button>
                                        ...
                                        <Table.Td class="first:rounded-l-md last:rounded-r-md w-40 bg-white border-b-0 dark:bg-darkmode-600 shadow-[20px_3px_20px_#0000000b] whitespace-nowrap">
                                            <div
                                                ...
                                                'flex items-center justify-center',
                                                    { 'text-success': data.status_product==0 },
                                                    { 'text-danger': data.status_product==1 }
                                                ]"
                                            >
                                                <!-- <Lucide icon="CheckSquare" class="w-4 h-4 mr-2" /> -->
                                                <div v-if="data.status_product==0">Active</div>
                                                <div v-else-if="data.status_product==1">Non Active</div>
                    ...
                    <Dialog.Description  class="grid grid-cols-12 gap-4 gap-y-3">
                        <div class="col-span-12 sm:col-span-6 xl:col-span-6 intro-y">
                            <FormInline class="mt-2">
                                <FormLabel  htmlFor="horizontal-form-2" class="sm:w-40" style="text-align: left;"> Product Code</FormLabel>
                                <FormInput type="text" class="col-span-8" v-model="state.data.kode_product" placeholder="Insert Product Code"/>
                            </FormInline>

                            <FormInline class="mt-2">
                                <FormLabel  htmlFor="horizontal-form-2" class="sm:w-40" style="text-align: left;"> Product Name</FormLabel>
                                <FormInput type="text" class="col-span-8" v-model="state.data.product_name" placeholder="Insert Product Name"/>
                            </FormInline>
                        ...
                        <div class="col-span-12 sm:col-span-6 xl:col-span-6 intro-y">
                            ...
                            <FormInline class="mt-2">
                                ...
                                <FormSelect v-model="state.data.status_product" class="w-full">
...
<script setup>
    ...
    const tampilData = async (page = state.listData.current_page) => {
        ...
        if(user.roles[0].name=='superAdmin' || user.roles[0].name=='admin'){
            await Api.get('/admin/product?page=' + page+ '&q=' + search.value)
            .then(response => {
            ...
            await Api.get('/sales/product?page=' + page+ '&q=' + search.value)
            .then(response => {
                state.listData = response.data.data
                loading.value = false
            }).catch(error => {
    ...
    const edit = async (id) => {
        ...
        .then(response => {
            ...
            state.data.kode_product=state.datas[0].kode_product
            state.data.product_name=state.datas[0].product_name
            state.data.kategori=state.datas[0].kategori
            state.data.status_product=state.datas[0].status_product
            action.value ='Update'
            AddModal.value = true
            loadingsycn.value = false
            basicModalPreview.value = false
    ...
    const save = async () => {
        ...
        if (action.value=='Add New') {
            await Api.post('/admin/product', {
                kode_product : state.data.kode_product,
                product_name : state.data.product_name,
                kategori : state.data.kategori,
                status_product : state.data.status_product
            }).then(async (response) => {
                loadingsycn.value = false
                basicModalPreview.value = false
            ...
            await Api.put('/admin/product', {
                ...
                pesan.value = "Success Update Product"
                setSuccessModalPreview(true)
                loadingsycn.value = false
                basicModalPreview.value = false
                AddModal.value = false
