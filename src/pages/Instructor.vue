    ...
    <div class="grid grid-cols-12 gap-6 mt-5">
        <div class="col-span-12 2xl:col-span-4">
            <div class="flex flex-wrap items-center col-span-12  intro-y sm:flex-nowrap">
                ...
                <RouterLink :to="{ name: 'Instructor' }" class="flex items-center pr-4 text-primary mt-5"><Lucide icon="ChevronRight" class="w-4 h-4 mr-3" /> Master Instructor</RouterLink>
            </div>
        
        </div>
        <div class="col-span-12 2xl:col-span-4">
            <h2 class="mt-4 text-lg font-medium intro-y items-center text-center">List Data Want To Be Yoga Instructor ?</h2>
        </div>
        <div class="col-span-12 2xl:col-span-4">
                    ...
                    <div class="items-center sm:flex sm:mr-4">
                        <Button variant="primary" class="mr-2 shadow-md" @click="add">
                            <span class="flex items-center justify-center w-40 h-5">
                                <Lucide icon="Plus" class="w-4 h-4" /> Add New Instructor
                            </span>
                        </Button>
                                    ...
                                    <Table.Tr class="intro-x" v-for="(data, index) in state.listData.data" :key="data.id" >
                                        ...
                                        <Table.Td class="first:rounded-l-md last:rounded-r-md w-40 bg-white border-b-0 dark:bg-darkmode-600 shadow-[20px_3px_20px_#0000000b] whitespace-nowrap">
                                            ...
                                            >
                                                ...
                                                <div v-if="data.status==0">Active</div>
                                                <div v-else-if="data.status==1">Non Active</div>
    ...
    <Dialog  size="xl" :open="AddModal"  @close="setAddModal(false)" >
                ...
                <Dialog.Panel>
                    ...
                    <Dialog.Description  class="grid grid-cols-12 gap-4 gap-y-3">
                        ...
                        <FormInline class="mt-2">
                            <FormLabel  htmlFor="horizontal-form-2" class="sm:w-40" style="text-align: left;"> Desc</FormLabel>
...
<script setup>
    ...
    const tampilData = async (page = state.listData.current_page) => {
        ...
        if (user.roles[0].name=='superAdmin' || user.roles[0].name=='admin') {
            url = Api.get('/admin/instructor?page=' + page+ '&q=' + search.value)
        }else{
            url = Api.get('/sales/instructor?page=' + page+ '&q=' + search.value)
        }
    ...
    const edit = async (id) => {
        ...
        let url=''
        if (user.roles[0].name=='superAdmin' || user.roles[0].name=='admin') {
            url = Api.get('/admin/instructor', {
                params: {
                    id : id
                } 
            })
        }else{
            url = Api.get('/sales/instructor', {
                params: {
                    id : id
                } 
            })
        ...
        await url.then(response => {
            ...
            state.data.status=state.datas[0].status
            action.value ='Update'
            AddModal.value = true
            loadingsycn.value = false
            basicModalPreview.value = false
    ...
    const save = async () => {
        ...
        if (action.value=='Add New') {
            let url=''
            if (user.roles[0].name=='superAdmin' || user.roles[0].name=='admin') {
                url = Api.post('/admin/instructor', {
                    title : state.data.title,
                    desc : state.data.desc,
                    status : state.data.status,
                })
            }else{
                url = Api.post('/sales/instructor', {
                    title : state.data.title,
                    desc : state.data.desc,
                    status : state.data.status,
                })
