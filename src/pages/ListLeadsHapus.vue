    ...
    <div class="grid grid-cols-12 gap-6 mt-5">
        <div class="col-span-12 2xl:col-span-4">
            <div class="flex flex-wrap items-center col-span-12  intro-y sm:flex-nowrap">
                ...
                <RouterLink :to="{ name: 'ListLeadsHapus' }" class="flex items-center pr-4 text-primary mt-5"><Lucide icon="ChevronRight" class="w-4 h-4 mr-3" /> List Data CRM</RouterLink>
            </div>
                ...
                <Table class="border-spacing-y-[10px] border-separate -mt-2">
                    <Table.Thead class="bg-primary text-white">
                        <Table.Tr>
                            ...
                            <Table.Th class="text-center border-b-0 whitespace-nowrap"> Keterangan</Table.Th>
                            ...
                            <Table.Td class="first:rounded-l-md last:rounded-r-md w-40 bg-white border-b-0 dark:bg-darkmode-600 shadow-[20px_3px_20px_#0000000b] whitespace-nowrap">
                                <div
                                    ...
                                    'flex items-center justify-center',
                                        { 'text-danger': data.status_crm==5 }
                                    ]"
                                >
                                    <!-- <Lucide icon="CheckSquare" class="w-4 h-4 mr-2" /> -->
                                    <div v-if="data.status_crm==5">Hapus Leads</div>
...
<script setup>
    ...
    const sales=ref(route.params.sales)
    const image = ref("")
    ...
    const tampilData = async (page = state.listData.current_page) => {
        ...
        if (user.roles[0].name=='superAdmin' || user.roles[0].name=='admin') {
            await Api.get('/admin/crm?page=' + page+ '&q=' + search.value + '&status=' + status.value+ '&tgl=' + tgl.value+ '&sales=' + sales.value)
            .then(response => {
        ...
            }
            ...
            await Api.get('/sales/crm?page=' + page+ '&q=' + search.value + '&status=' + status.value+ '&tgl=' + tgl.value+ '&sales=' + sales.value)
            .then(response => {
                state.listData = response.data.data
                state.listSource = response.data.source
                state.listKategori = response.data.kategori
