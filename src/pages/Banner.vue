    ...
    <div class="grid grid-cols-12 gap-6 mt-5">
        <div class="col-span-12 2xl:col-span-4">
            <div class="flex flex-wrap items-center col-span-12  intro-y sm:flex-nowrap">
                ...
                <RouterLink :to="{ name: 'Banner' }" class="flex items-center pr-4 text-primary mt-5"><Lucide icon="ChevronRight" class="w-4 h-4 mr-3" /> Master Banner</RouterLink>
            </div>
        
        </div>
        <div class="col-span-12 2xl:col-span-4">
            <h2 class="mt-4 text-lg font-medium intro-y items-center text-center">List Data Banner</h2>
        </div>
        <div class="col-span-12 2xl:col-span-4">
                    ...
                    <div class="items-center sm:flex sm:mr-4">
                        <Button variant="primary" class="mr-2 shadow-md" @click="add">
                            <span class="flex items-center justify-center w-40 h-5">
                                <Lucide icon="Plus" class="w-4 h-4" /> Add New Banner
                            </span>
                        </Button>
                                ...
                                <Table.Tbody>
                                    ...
                                    <Table.Tr class="intro-x" v-for="(data, index) in state.listData.data" :key="data.id" v-else>
                                        ...
                                        <Table.Td class="first:rounded-l-md last:rounded-r-md bg-white border-b-0 dark:bg-darkmode-600 shadow-[20px_3px_20px_#0000000b]  text-center">
                                            ...
                                            <a-image
                                                :width="200"
                                                :src="'https://login.yogafitidonline.com/api/storage/banner/'+data.file"
                                            />
                                        </Table.Td>
                                        ...
                                        <Table.Td class="first:rounded-l-md last:rounded-r-md w-40 bg-white border-b-0 dark:bg-darkmode-600 shadow-[20px_3px_20px_#0000000b] whitespace-nowrap">
                                            <div
                                                ...
                                                'flex items-center justify-center',
                                                    { 'text-success': data.status_banner==0 },
                                                    { 'text-danger': data.status_banner==1 }
                                                ]"
                                            >
                                                <!-- <Lucide icon="CheckSquare" class="w-4 h-4 mr-2" /> -->
                                                <div v-if="data.status_banner==0">Active</div>
                                                <div v-else-if="data.status_banner==1">Non Active</div>
            ...
            <Dialog.Description  class="grid grid-cols-12 gap-4 gap-y-3">
                <div class="col-span-12 sm:col-span-12 xl:col-span-12 intro-y">
                    ...
                    <FormInline class="mt-2">
                        <FormLabel  htmlFor="horizontal-form-2" class="sm:w-20" style="text-align: left;"> Title</FormLabel>
                        <a-input style="width:100%;border-radius:7px;border-color: rgb(217 217 217);padding: 4px 9px;" v-model:value="state.data.title" placeholder="Input Title"/>
                    </FormInline> 
                    ...
                    <FormInline class="mt-2">
                        ...
                        <a-select v-model:value="state.data.status"  size="large" style="width: 100%"  placeholder="Pilih Status">
...
<script setup>
    ...
    const tampilData = async (page = state.listData.current_page) => {
        ...
        const token = localStorage.getItem('token_yogafit')
        Api.defaults.headers.common['Authorization'] = "Bearer " +token
        await Api.get('/admin/banner?page=' + page+ '&q=' + search.value)
        .then(response => {
    ...
    const edit = async (data) => {
        ...
        state.data.status=data.status_banner
        action.value ='Update'
        AddModal.value = true
        loadingsycn.value = false
        basicModalPreview.value = false
    ...
    const save = async () => {
        ...
        if (action.value=='Add New') {
            ...
            await Api.post('/admin/banner', formdata, {
                ...
                pesan.value = "Banner Success Added"
                setAddModal(false)
                loadingsycn.value = false
                basicModalPreview.value = false
                tampilData()
            ...
            await Api.post('/admin/banner_update', formdata, {
                headers: {
                    'Content-Type': 'multipart/form-data',
                },
            }).then(async (response) => {
                ...
                setSuccessModalPreview(true)
                pesan.value = "Banner Success Updated"
                setAddModal(false)
                loadingsycn.value = false
                basicModalPreview.value = false
                tampilData()
