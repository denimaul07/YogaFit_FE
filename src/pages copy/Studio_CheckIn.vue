<script setup>
    ...
    const tampilSchedule = async ()=>{
        ...
        if (user.roles[0].name=='studio') {
            await Api.get('/studio/get_schedule')
            .then(response => {
    ...
    const tampilData = async (page=state.listData.current_page) => {
        ...
        Api.defaults.headers.common['Authorization'] = "Bearer " +token
        if (user.roles[0].name=='studio') {
            await Api.get('/studio/schedule?page=' + page+ '&q=' + search.value + '&status=' + status.value+  '&studio=' + studio.value+ '&tgl=' + tgl.value)
            .then(response => {
        ...
            }
            ...
            await Api.get('/admin/schedule?page=' + page+ '&q=' + search.value + '&status=' + status.value+  '&studio=' + studio.value+ '&tgl=' + tgl.value)
            .then(response => {
                state.listData = response.data.data
                loading.value = false
            }).catch(error => {
    ...
    const searchingMember = useDebounceFn(() => {
        const token = localStorage.getItem('token_iss')
        Api.defaults.headers.common['Authorization'] = "Bearer " +token
        Api.get('/studio/contract?search=' + searchMember.value)
        .then(response => {
            state.listMember = response.data.data
    ...
    const searchingBooking = useDebounceFn(() => {
        ...
        const token = localStorage.getItem('token_iss')
        Api.defaults.headers.common['Authorization'] = "Bearer " +token
        Api.get('/studio/booking?id=' + state.data.id+ '&q=' + searchBooking.value)
        .then(response => {
            state.listBooking = response.data.data
            loadingsycn.value = false
            basicModalPreview.value = false
    ...
    const searchingPresent = useDebounceFn(() => {
        ...
        const token = localStorage.getItem('token_iss')
        Api.defaults.headers.common['Authorization'] = "Bearer " +token
        Api.get('/studio/booking?id=' + state.data.id+ '&qpresent=' + searchPresent.value)
        .then(response => {
            state.listBookingPresent = response.data.present
            loadingsycn.value = false
            basicModalPreview.value = false
    ...
    const searchingAbsent = useDebounceFn(() => {
        ...
        const token = localStorage.getItem('token_iss')
        Api.defaults.headers.common['Authorization'] = "Bearer " +token
        Api.get('/studio/booking?id=' + state.data.id+ '&qabsent=' + searchAbsent.value)
        .then(response => {
            state.listBookingAbsent = response.data.absent
            loadingsycn.value = false
            basicModalPreview.value = false
    ...
    watch(qr, (newQuestion, oldQuestion) => {
        ...
        const token = localStorage.getItem('token_iss')
        Api.defaults.headers.common['Authorization'] = "Bearer " +token
        Api.get('/studio/my_profile?id=' + qr.value)
        .then(response => {
            state.listProfile = response.data.data
            Api.get('/studio/my_contract?id=' + state.listProfile.idusers)
            .then(responses => {
                state.listContract = responses.data.data
                Api.get('/studio/my_booking?id=' + state.listProfile.idusers)
                .then(responsesss => {
            ...
            <Tab.Panels class="mt-5">
                ...
                <div class="p-5 box">
                        ...
                        <FormInline class="mt-5">
                            ...
                            <input type="text" ref="qrInput" v-model="qr" placeholder="Scan QR" class="transition duration-200 ease-in-out w-full text-sm border-slate-200 shadow-sm rounded-md placeholder:text-slate-400/90 focus:ring-4 focus:ring-primary focus:ring-opacity-20 focus:border-primary focus:border-opacity-40 dark:bg-darkmode-800 dark:border-transparent dark:focus:ring-slate-700 dark:focus:ring-opacity-50 dark:placeholder:text-slate-500/80"/>
                        </FormInline>

                        <div class="grid grid-cols-12 gap-6 mt-5">
                            <div class="flex flex-col-reverse col-span-12 lg:col-span-4 2xl:col-span-3 lg:block" v-if="state.listProfile===null">
                                No Members Found
                            </div>
                            
                            <div class="flex flex-col-reverse col-span-12 lg:col-span-4 2xl:col-span-3 lg:block" v-else>
                                <div class="mt-5 intro-y box lg:mt-0">
                                    ...
                                    <a class="flex items-center mt-5" href="">
                                        <Lucide icon="Phone" class="w-4 h-4 mr-2" /> {{ state.listProfile.no_telp }}
                                    </a>
                                    <a class="flex items-center mt-5" href="">
